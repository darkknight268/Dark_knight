...
# Sync API keys to amass datasources.yaml if env vars are set
AMASS_DS="$HOME/.config/amass/datasources.yaml"
if [ -f "$AMASS_DS" ] && { [ -n "$CHAOS_API_KEY" ] || [ -n "$SHODAN_API_KEY" ]; }; then
  cat > "$AMASS_DS" <<- EOF
global_options:
  minimum_ttl: 1440
datasources:
EOF
  add_amass_key() { local name=$1 key=$2 val=$3; [ -n "$val" ] && cat >> "$AMASS_DS" <<- EOF
  - name: $name
    creds:
      account: 
        $key: $val
EOF
  ...
  add_amass_key "AlienVault" "apikey" "$ALIENVAULT_API_KEY"
  add_amass_key "BeVigil" "apikey" "$BEVIGIL_API_KEY"
  add_amass_key "BufferOver" "apikey" "$BUFFEROVER_API_KEY"
  add_amass_key "Chaos" "apikey" "$CHAOS_API_KEY"
  [ -n "$CENSYS_API_ID" ] && add_amass_key "Censys" "apikey" "$CENSYS_API_ID:$CENSYS_API_SECRET"
  add_amass_key "FullHunt" "apikey" "$FULLHUNT_API_KEY"
  add_amass_key "GitHub" "apikey" "$GITHUB_TOKEN"
  add_amass_key "LeakIX" "apikey" "$LEAKIX_API_KEY"
  add_amass_key "Shodan" "apikey" "$SHODAN_API_KEY"
  add_amass_key "VirusTotal" "apikey" "$VIRUSTOTAL_API_KEY"
  add_amass_key "WhoisXMLAPI" "apikey" "$WHOISXML_API_KEY"
  add_amass_key "FOFA" "apikey" "$FOFA_API_KEY"
  echo -e "  ${D}Synced API keys to amass datasources.yaml${O}"
fi
...
phase_header() {
  local num=$1 title=$2
  echo -e ""
  echo -e "  ${C}✦${O} ${W}PHASE $num/$TOTAL_PHASES${O} ${D}⬡${O} ${C}$title${O}"
  draw_progress "$num" "$TOTAL_PHASES"
  echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
}
...
summary_item() {
  ...
  printf "  ${D}│${O}  %-18s ${D}▸${O}  ${W}%s${O}\n" "$label" "$value"
}

mkdir -p "$OUTPUT_DIR"
echo -e ""
echo -e "  ${C}⚡ ${W}BUG HUNTER${O} ${D}│${O} ${G}RECON PIPELINE${O}"
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
echo -e "  ${D}Target:${O}     ${W}$TARGET${O}"
echo -e "  ${D}Workspace:${O}  ${C}$OUTPUT_DIR${O}"
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
...
echo "    ${Y}▸${O} Shodan (API)..."
[ -n "$SHODAN_API_KEY" ] && curl -s "https://api.shodan.io/dns/domain/$TARGET?key=$SHODAN_API_KEY" 2>/dev/null | jq -r '.subdomains[]' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_shodan.txt" || true

echo "    ${Y}▸${O} VirusTotal (API)..."
[ -n "$VIRUSTOTAL_API_KEY" ] && curl -s "https://www.virustotal.com/api/v3/domains/$TARGET/subdomains?limit=40" -H "x-apikey: $VIRUSTOTAL_API_KEY" 2>/dev/null | jq -r '.data[].id' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_virustotal.txt" || true

echo "    ${Y}▸${O} SecurityTrails (API)..."
[ -n "$SECURITYTRAILS_API_KEY" ] && curl -s "https://api.securitytrails.com/v1/domain/$TARGET/subdomains" -H "APIKEY: $SECURITYTRAILS_API_KEY" 2>/dev/null | jq -r '.subdomains[]' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_securitytrails.txt" || true

echo "    ${Y}▸${O} URLScan.io (API)..."
curl -s "https://urlscan.io/api/v1/search/?q=domain:$TARGET" 2>/dev/null | jq -r '.results[].page.domain' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_urlscan.txt" || true

echo "    ${Y}▸${O} BeVigil (API)..."
[ -n "$BEVIGIL_API_KEY" ] && curl -s "https://osint.bevigil.com/api/$TARGET/subdomains/" -H "X-Access-Token: $BEVIGIL_API_KEY" 2>/dev/null | jq -r '.subdomains[]' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_bevigil.txt" || true

echo "    ${Y}▸${O} FullHunt (API)..."
[ -n "$FULLHUNT_API_KEY" ] && curl -s "https://fullhunt.io/api/v1/domain/$TARGET/subdomains" -H "X-API-KEY: $FULLHUNT_API_KEY" 2>/dev/null | jq -r '.subdomains[]' 2>/dev/null | sort -u > "$OUTPUT_DIR/subs_fullhunt.txt" || true

echo "    ${Y}▸${O} RapidDNS..."
curl -s "https://rapiddns.io/subdomain/$TARGET?full=1" 2>/dev/null | grep -oE 'https?://[^"<>]+\.$TARGET' | sed 's|https\?://||' | sort -u > "$OUTPUT_DIR/subs_rapiddns.txt" || true
...
echo ""
echo -e "  ${C}✦${O} ${W}PHASE 2b${O} ${D}⬡${O} ${C}Quick Wins — Background Scanning on Subs${O}"
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
echo ""

echo "    ${Y}▸${O} Firing off background nuclei scans (continues to Phase 3)..."
(
  set +e
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/exposures/configs/phpinfo-files.yaml" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_phpinfo.json" 2>/dev/null
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/exposures/configs/" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_configs.json" 2>/dev/null
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/exposed-panels/" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_panels.json" 2>/dev/null
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/default-logins/" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_logins.json" 2>/dev/null
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/technologies/" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_tech.json" 2>/dev/null
  nuclei -l "$OUTPUT_DIR/subs.uniq" -t "$NUCLEI_TEMPLATES/http/technologies/springboot-actuator.yaml" -silent -jsonl -o "$OUTPUT_DIR/bugs_quick_springboot.json" 2>/dev/null
) &>/dev/null &
QUICK_PID=$!
echo -e "  ${C}┃${O}  ${D}  Background PID: $QUICK_PID${O}"
echo -e "  ${C}┃${O}  ${D}  Results will appear in bugs_quick_*.json when done${O}"
...
# ==================================================================
echo -e ""
echo -e "  ${G}✔${O} ${W}RECON PIPELINE COMPLETE${O}"
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
...
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
echo -e "  ${D}Output Directory ${D}▸${O} ${C}$OUTPUT_DIR${O}"
echo -e "  ${D}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${O}"
